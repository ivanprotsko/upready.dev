...
import { NavItem } from "@/components/shadcn-space/blocks/dashboard-shell-02/app-sidebar";
        ...
        <SidebarGroup key={item.title} className="p-0">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                tooltip={item.title}
                className={cn("rounded-lg text-sm px-3 py-2 h-9 ", isActive ? "bg-primary hover:bg-primary dark:bg-blue-500 text-white dark:hover:bg-blue-500 hover:text-white" : "")}
              >
                {item.icon && <item.icon />}
